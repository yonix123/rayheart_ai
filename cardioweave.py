#!/usr/bin/env python3
"""
CardioWeave — Single-channel ECG + SpO2 + Heart Rate
=====================================================
Hardware : 1x AD8232 + ADS1115 (A2) + MAX30102 + Raspberry Pi
LO pins  : GPIO 17 (LO+), GPIO 27 (LO-)
...
LO_PINS    = {"p": 17, "n": 27}          # Single AD8232 LO pins
LEAD_NAME  = "Lead I (RA–LA)"
LEAD_COLOR = "#00ff88"

# ADS1115 gain=2 → ±2.048V range → 62.5µV/LSB resolution
# Much better than gain=1 (±4.096V, 125µV/LSB) for millivolt ECG signals
ADS_GAIN = 2

# DC midpoint of AD8232 output ≈ VCC/2 = 1.65V at 3.3V supply
# With gain=2 and 3.3V supply the output sits around 825mV in voltage terms
ECG_DC_OFFSET_MV = 825.0
...
def extract_features(sig, fs=100):
    """Extract time-domain + frequency-domain features from a single ECG lead."""
    features = []
    ...
    # ST segment (samples after QRS peak)
    pk = int(np.argmax(np.abs(sig)))
    ss = min(pk + 6,  len(sig) - 1)
    se = min(pk + 12, len(sig))
    st = sig[ss:se]
    features.extend([
        np.mean(st) if len(st) > 0 else 0.0,
        np.std(st)  if len(st) > 0 else 0.0,
    ])
...
class MIPredictor:
    ...
    def _load(self):
        ...
        if not files:
            raise FileNotFoundError("No model found in " + MODEL_DIR)
    ...
    def predict(self, ecg_buf):
        if len(ecg_buf) < 100:
            return {"probability": 0.0, "alert": False, "message": "Not enough data"}
        from scipy.signal import resample as sp_resample
        sig    = np.array(ecg_buf, dtype=np.float64)
        target = int(len(sig) * 100 / SAMPLE_RATE)
        sig    = sp_resample(sig, target)
...
# =============================================================================
#  HARDWARE INTERFACE  (single AD8232 on ADS1115 A2)
# =============================================================================
...
class ECGHardware:
    def __init__(self):
        ...
        self.ads.gain = ADS_GAIN          # gain=2 → ±2.048V, finer resolution
        self.ch = AnalogIn(self.ads, 2)   # AD8232 OUTPUT → ADS1115 A2
        GPIO.setmode(GPIO.BCM)
        for pin in LO_PINS.values():
            GPIO.setup(pin, GPIO.IN)
        print(f"[HW] ADS1115 ready (gain={ADS_GAIN}, channel A2)")

    def read(self):
        lo = any(GPIO.input(p) for p in LO_PINS.values())
        if lo:
            return None, True
        v = self.ch.voltage * 1000.0      # convert to millivolts
        return v, False
...
class MockHardware:
    def __init__(self):
        self._t = 0.0
        print("[MOCK] Using synthetic single-channel ECG data")
...
# =============================================================================
#  ACQUISITION THREAD  (single channel)
# =============================================================================
...
class AcquisitionThread(threading.Thread):
    def __init__(self, hw):
        ...
        self.disp_ecg = deque([0.0] * win, maxlen=win)

        inf_win = INFERENCE_EVERY * SAMPLE_RATE
        self.inf_ecg = deque(maxlen=inf_win)
        ...
        self.zi_bp = sosfilt_zi(sos_bp) * 0
        self.zi_n  = sosfilt_zi(sos_n)  * 0
    ...
    def _process(self, v):
        v -= ECG_DC_OFFSET_MV             # remove DC bias
        s, self.zi_bp = sosfilt(self.sos_bp, [v], zi=self.zi_bp)
        s, self.zi_n  = sosfilt(self.sos_n,  s,   zi=self.zi_n)
        f = float(s[0])
        with self.lock:
            self.disp_ecg.append(f)
            self.inf_ecg.append(f)
            if self.recording:
                self.rec_ecg.append(f)

    def get_display(self):
        with self.lock:
            return list(self.disp_ecg)

    def get_inference_window(self):
        with self.lock:
            return list(self.inf_ecg)
    ...
    def stop_recording(self):
        with self.lock:
            self.recording = False
            return list(self.rec_ecg), self.rec_start
...
# =============================================================================
#  EDF STORAGE  (single channel)
# =============================================================================

def save_edf(ecg, start_time):
    os.makedirs(RECORDINGS_DIR, exist_ok=True)
    ts   = start_time.strftime("%Y%m%d_%H%M%S")
    path = os.path.join(RECORDINGS_DIR, f"ecg_{ts}.edf")
    f    = pyedflib.EdfWriter(path, 1, file_type=pyedflib.FILETYPE_EDFPLUS)
    f.setStartdatetime(start_time)
    sig = np.array(ecg)
    mx  = float(np.max(np.abs(sig)) + 0.1) if len(sig) else 5.0
    f.setSignalHeader(0, {
        "label": "ECG_LeadI", "dimension": "mV",
    ...
    f.writeSamples([np.array(ecg, dtype=np.float64)])
    f.close()
...
class InferenceThread(threading.Thread):
    ...
    def run(self):
        ...
        while self.running:
            ...
            ecg      = self.acq.get_inference_window()
            result   = self.predictor.predict(ecg)
...
class ECGDisplay:
    def __init__(self, acq, inference, ws, max_thread):
        ...
        # Single ECG waveform spanning full width
        ax = self.fig.add_subplot(gs[0, :])
        ax.set_facecolor("#0a0e0e")
        ax.set_xlim(0, DISPLAY_WINDOW)
        ax.set_ylim(-50, 50)
        ax.set_ylabel(LEAD_NAME, color=LEAD_COLOR, fontsize=9)
        ...
        for sp in ax.spines.values():
            sp.set_color("#223333")
        self.line, = ax.plot(x, np.zeros(n), color=LEAD_COLOR,
                             linewidth=0.9, antialiased=True)

        # HR panel
        ax_hr = self.fig.add_subplot(gs[1, 0])
        ...
        # SpO2 panel
        ax_spo2 = self.fig.add_subplot(gs[1, 1])
    ...
    def _toggle_recording(self):
        if not self.is_recording:
            ...
            self.is_recording = False
            ecg, start = self.acq.stop_recording()
            self.txt_rec.set_text("")
            def _save():
                path = save_edf(ecg, start)
    ...
    def _animate(self, _frame):
        ...
        return [self.line, self.txt_status, self.txt_lo, self.txt_ai,
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .header h1 { font-size: 26px; font-weight: 700; color: #F57C00; letter-spacing: -0.5px; }
  ...
  .card { background: #fff; border-radius: 16px; padding: 18px; border: 0.5px solid #ececec; }
  .vitals-row { display: flex; gap: 14px; }
  .vital-card { flex: 1; text-align: center; }
  .card-label {
    font-size: 11px; font-weight: 600; color: #bbb;
    text-transform: uppercase; letter-spacing: 0.8px; margin-bottom: 10px;
  }
  .vital-value { font-size: 32px; font-weight: 700; color: #222; line-height: 1; }
  .vital-unit  { font-size: 12px; color: #999; font-weight: 600; margin-top: 4px; }
  .status-row  { display: flex; align-items: center; gap: 8px; }
  .dot         { width: 8px; height: 8px; border-radius: 50%; background: #4CAF50; }
  .dot.alert   { background: #F44336; }
  .status-text { font-size: 15px; font-weight: 500; color: #222; }
  .prob-big    { font-size: 56px; font-weight: 700; color: #222; line-height: 1; margin: 8px 0 4px; }
  .prob-big.alert  { color: #F44336; }
  .prob-big.normal { color: #4CAF50; }
  .prob-sub    { font-size: 13px; color: #999; }
  .bar-wrap    { background: #f0f0f0; border-radius: 8px; height: 8px; margin-top: 14px; overflow: hidden; }
  .bar-fill    { height: 8px; border-radius: 8px; background: #4CAF50; width: 0%; transition: width 0.6s ease, background 0.4s; }
  .bar-fill.alert { background: #F44336; }
  .message-box { background: #fff8f0; border: 1px solid #FFE0B2; border-radius: 12px; padding: 14px 16px; display: none; }
  .message-box.visible { display: block; }
  .message-box p { font-size: 14px; color: #E65100; font-weight: 500; line-height: 1.5; }
  .info-row    { display: flex; justify-content: space-between; align-items: center; padding: 10px 0; border-bottom: 0.5px solid #f5f5f5; }
  .info-row:last-child { border-bottom: none; }
  .info-label  { font-size: 13px; color: #999; }
  .info-val    { font-size: 13px; font-weight: 500; color: #333; }
  .conn-badge  { display: inline-flex; align-items: center; gap: 5px; padding: 4px 10px; border-radius: 20px; font-size: 12px; font-weight: 500; background: #E8F5E9; color: #2E7D32; }
  .conn-badge.disconnected { background: #ffeee8; color: #c0392b; }
  .footer      { padding: 16px 20px 32px; background: #f7f7f7; text-align: center; }
...
<div class="content">
  ...
  <div class="card">
    ...
    <div class="bar-wrap"><div class="bar-fill" id="prob-bar"></div></div>
  </div>
...
function setConnected(on) {
  ...
  if (on) {
    ...
    badge.className = "conn-badge"; badge.textContent = "Live";
  } else {
    dot.className = "dot alert"; text.textContent = "Disconnected";
    badge.className = "conn-badge disconnected"; badge.textContent = "Offline";
  }
}
...
function connect() {
  ...
  ws.onmessage = e => {
    ...
    if (d.type === "init") {
      document.getElementById("threshold-val").textContent = Math.round(d.threshold * 100) + "%";
    }
    if (d.type === "vitals" || d.type === "inference") {
      if (d.hr   !== undefined) document.getElementById("hr-val").textContent   = d.hr   > 0 ? d.hr   : "--";
      if (d.spo2 !== undefined) {
        const el = document.getElementById("spo2-val");
        el.textContent  = d.spo2 > 0 ? d.spo2 : "--";
        el.style.color  = (d.spo2 > 0 && d.spo2 < 95) ? "#F44336" : "#222";
      }
    }
    if (d.type === "inference") {
      ...
      if (d.alert) {
        probEl.className = "prob-big alert"; barEl.className = "bar-fill alert";
        alertBox.className = "message-box visible";
        subEl.textContent = "Elevated risk detected";
        statusEl.textContent = "ALERT"; statusEl.style.color = "#F44336";
      } else {
        probEl.className = "prob-big normal"; barEl.className = "bar-fill";
        alertBox.className = "message-box";
        subEl.textContent = d.message;
        statusEl.textContent = "Normal"; statusEl.style.color = "#4CAF50";
      }
...
def main():
    print("=" * 60)
    print("  CardioWeave — Single-channel ECG + SpO2 + HR + MI")
    print(f"  Sample rate    : {SAMPLE_RATE} SPS")
    print(f"  ADS1115 gain   : {ADS_GAIN}  (±2.048V, 62.5µV/LSB)")
    print(f"  DC offset      : {ECG_DC_OFFSET_MV} mV")
